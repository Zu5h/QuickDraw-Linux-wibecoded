...
if (window.QuickDrawLinux === true) {
    document.body.classList.add("QuickDrawLinux");
}

function PostMessage(message) {
    if (window.QuickDrawWindows === true) {
        window.chrome.webview.postMessage(message)
    } else if (window.QuickDrawMacOs === true || window.QuickDrawLinux === true) {
...
class Slideshow {
    ...
    resetInterval() {
        ...
        let timer = document.getElementById("slideshow-timer");
        this.currentInterval = setInterval((() => {
            progress.style.width = `${this.currentTimer.timePercentElapsed}%`;
            let remaining = Math.max(0, Math.ceil(this.currentTimer.timeRemaining / 1000));
